...
import os
import gzip, pickle
# Peter's machine doesn't handle cuda&tensorflow well
...
def calculate_fid_stats(images, batch_size=4, debug=False):
    if debug: print("calculte FID stats...", end=" ", flush=True)
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        mu, sigma = calculate_activation_statistics(images, sess, batch_size=batch_size)
    if debug: print("finished")
    return mu, sigma

def calculate_fid(gen_images_path, real_images_path, width=256, debug=False):
    """
    ...
    images = load_images_from_path(real_images_path)
    if debug: print("%d real images found and loaded" % len(images))
    ...
    mu_real, sigma_real = calculate_fid_stats(images, debug=debug)
    

    images = load_images_from_path(gen_images_path)
    if debug: print("%d generated images found and loaded" % len(images))

    mu_gen, sigma_gen = calculate_fid_stats(images, debug=debug)
    
    fid_value = calculate_frechet_distance(mu_gen, sigma_gen, mu_real, sigma_real)
    if debug: print("FID: %s" % fid_value)
    return fid_value
''' End code modified by Peter '''
